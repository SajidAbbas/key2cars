...
<a href="{{ route('addModel') }}" class="btn bg-blue">
...
Delete (selected)
    ...
    <section class="content">
      ...
      <div class="box box-default" style="padding: 40px;">
        ...
        <form method="post" action="{{route('searchModel')}}">
          {{ csrf_field() }}
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
            <div class="row">
                <div class="col-md-6">
            <div class="input-group input-group-md">
                <td>    <select name="make" class="form-control select2" style="width: 100%;"  >
                  <option selected="selected" disabled value="">By Make</option>
            ...
            <div class="box-body">
               <table class="table table-bordered">
                <tr>
                  ...
                  <th style="width: 150px;">Body Type</th>
                ...
                <tr>
                  ...
                  <td style=" vertical-align: middle"> {{$d->vehicle_type}}</td>
                   <td style=" vertical-align: middle"> {{$d->body_type}}</td>
                                    
                  <td style=" vertical-align: middle"> {{$d->make}}</td>                  
                  <td style=" vertical-align: middle"> {{$d->title}}</td>  
                  <td style=" vertical-align: middle"> {{$d->popular}}</td>
                  <td style=" vertical-align: middle"><a class="btn btn-default" href="{{Route('editModel', ['id' => $d->id])}}"><i class="fa fa-pencil"></i> Edit</a>
                  </td>
                </tr>
                <?php $serial++;?>
                @endforeach
...
function deleteRecord(ids)
  {
    ...
    buttons: {
        confirm: function () {
      var data = { 'id': ids };
        $.get('deleteModel', data, function (data) {
