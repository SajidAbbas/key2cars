...
<div id="discount-edit" class="nav-tabs-custom">
    ...
    <form id="form" class="form-horizontal " method="POST" action="{{ route('updateDiscount') }}" enctype = "multipart/form-data" >
                        {{ csrf_field() }}
...
<div class='input-group input-group-required'>
    <input class="form-control text-box single-line" style='width:50%' id="Name" name="name" type="text" value="{{$discount->name}}" /></div>
 @if ($errors->has('name'))
...
<select class="form-control select2"  style='width:50%'  id="DiscountTypeId" name="discount_type">
    @if($discount->discount_type==\App\ShopDiscount::order_discount)
    <option value="{{\App\ShopDiscount::order_discount}}" selected>Assigned to order total</option>
...
@elseif($discount->discount_type==\App\ShopDiscount::product_discount)
    <option value="{{\App\ShopDiscount::order_discount}}" >Assigned to order total</option>
<option value="{{\App\ShopDiscount::product_discount}}" selected>Assigned to products</option>
<option value="{{\App\ShopDiscount::category_discount}}">Assigned to categories</option>

@elseif($discount->discount_type==\App\ShopDiscount::category_discount)
    <option value="{{\App\ShopDiscount::order_discount}}" >Assigned to order total</option>
<option value="{{\App\ShopDiscount::product_discount}}">Assigned to products</option>
<option value="{{\App\ShopDiscount::category_discount}}" selected>Assigned to categories</option>
...
<div class="col-md-9">
<input type="number" class='form-control' style='width:50%' id="DiscountAmount" name="discount" value="{{$discount->discount}}" />
...
<div class="col-md-9">
<input type="number" class='form-control' style='width:50%'  id="MaximumDiscountAmount" name="max_discount_amount" value="{{$discount->max_discount_amount}}" />
...
<div class="col-md-9">
<input  class='form-control' style='width:50%' type='date' name="start_date" value='{{$discount->start_date}}'/>
...
<div class="col-md-9">
<input  class='form-control' style='width:50%' type='date' name="end_date" value='{{$discount->end_date}}' />
...
<div class="col-md-3">
<div class="label-wrapper"><label class="control-label" for="Published">Status</label><div title='Check to publish this product (visible in store). Uncheck to unpublish (product not available in store).' class='ico-help'><i class='fa fa-question-circle'></i></div></div>
</div>
<div class="col-md-9">
<div class="btn-group btn-group-toggle" data-toggle="buttons">
  ...
  <label class="btn btn-primary ">
    ...
    @else
    <label class="btn btn-primary ">
    <input type="radio" name="status" autocomplete="off" value='1' >Active
  </label>
  <label class="btn btn-primary active">
      <input type="radio" name="status"  autocomplete="off" value='0' checked> InActive
  </label>
