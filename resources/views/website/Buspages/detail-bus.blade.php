      ...
      <div class="row ad-listing-template mt10" itemprop="itemOffered" itemscope="" itemtype="http://schema.org/Car">
        ...
        <div class="col-md-8">
			...
			<div  class="detail-title " >
              ...
              <h1 itemprop="name" style="text-align:right;">PKR: {{$d->price}}</h1>              
              </div>
              <div class="clear"></div>
...
<div class="clearfix"></div>
  ...
  <ol class="m-l-15">
    ...
    @endforeach
          ...
          <section>
  <div class="container">
     <h2 class="text-uppercase">Similar Bus Ads</h2>
     <div class="row">
            
            	  @foreach($related_bus as $r_b)
                        
                  <div class="col-sm-3 m-bottom-20 ">
                          <div class="port_item xs-m-top-30 border">
                            <div class="port_img"> <a href="{{Route('/GetBusDetails', ['id' => $r_b->id])}}"><img class="img-responsive rel-featured"  src="{{URL::asset('images'.$r_b->url)}}" alt="" /></a>
                              @if($r_b->featured==1)
                              <div  class="absolute-featured"> <i class="fa fa-star"></i> FEATURED </div>
                            </div>
                            @endif
                            <div class="port_caption m-top-10 car-info">
                              <h3 ><a href="{{Route('/GetBusDetails', ['id' => $r_b->id])}}">{{$r_b->manufacture}} {{$r_b->model}} {{$r_b->model_version}}<span style="color:#223b7b">&nbsp; {{$r_b->year}}</span></a></h3>
                              <h6 class=""><i class="fa fa-map-marker"></i>&nbsp; {{$r_b->city}}</h6>
                              <h5 class="pull-left" style="color:#ff4436">PKR <span style="color:#ff4436">{{$r_b->price}}</span></h5>
                              <h5 class="pull-right">Condition: {{$r_b->condition}}</h5>
                              <div style="border-bottom:1px solid #DFDFDF; margin-bottom:10px;" class="clear"></div>
                              <a href="{{Route('/GetBusDetails', ['id' => $r_b->id])}}" class="btn-more pull-right"  style="text-decoration:none;">View More</a>
