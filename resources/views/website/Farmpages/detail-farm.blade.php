...
@section('title')
Farms for Sale in Pakistan | Key2Cars
@endsection
      ...
      <div class="row ad-listing-template mt10" itemprop="itemOffered" itemscope="" itemtype="http://schema.org/Car">
        ...
        <div class="col-md-8">
            ...
            <div class="well">
              ...
              <div class="pull-left">
              <h1 itemprop="name">{{$d->manufacture}} {{$d->model}} {{$d->model_version}} {{$d->model_year}}</h1>
              ...
              <h1 itemprop="name">PKR: {{$d->price}}</h1>              
              </div>
              <div class="clear"></div>
                ...
                <table width="100%" class="table table-bordered text-center table-engine-detail fs16">
                  <tbody>
                    <tr>
                      ...
                      <td>
                        <span class="engine-icon type"><i class="fa fa-flask"></i></span>
...
<div class="clearfix"></div>
  ...
  <ol class="m-l-15">
    ...
    @endforeach
          ...
          <section>
  <div class="container">
     <h2 class="text-uppercase">Similar Farm Ads</h2>
     <div class="row">
            
            	  @foreach($related_farm as $r_f)
                        
                  <div class="col-sm-3 m-bottom-20 ">
                          <div class="port_item xs-m-top-30 border">
                            <div class="port_img"> <a href="{{Route('/GetFarmDetails', ['id' => $r_f->id])}}"><img class="img-responsive rel-featured"  src="{{URL::asset('images'.$r_f->url)}}" alt="" /></a>
                              @if($r_f->featured==1)
                              <div  class="absolute-featured"> <i class="fa fa-star"></i> FEATURED </div>
                            </div>
                            @endif
                            <div class="port_caption m-top-10 car-info">
                              <h3 ><a href="{{Route('/GetFarmDetails', ['id' => $r_f->id])}}">{{$r_f->manufacture}} {{$r_f->model}} {{$r_f->model_version}}<span style="color:#223b7b">&nbsp; {{$r_f->year}}</span></a></h3>
                              <h6 class=""><i class="fa fa-map-marker"></i>&nbsp; {{$r_f->city}}</h6>
                              <h5 class="pull-left" style="color:#ff4436">PKR <span style="color:#ff4436">{{$r_f->price}}</span></h5>
                              <h5 class="pull-right">Condition: {{$r_f->condition}}</h5>
                              <div style="border-bottom:1px solid #DFDFDF; margin-bottom:10px;" class="clear"></div>
                              <a href="{{Route('/GetFarmDetails', ['id' => $r_f->id])}}" class="btn-more pull-right"  style="text-decoration:none;">View More</a>
