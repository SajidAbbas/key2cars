...
class BusController extends Controller
{
  ...
  public function __construct() {
    ...
    public function index()
    {
     ...
     $new_busses=$this->vehicle->getFeaturedOthers(\App\VehicleType::Bus);

      $city_vehicles_count=$this->city->getAllCityVehicles(\App\VehicleType::Bus);
      
      $manufacture_busses=$this->manufacture->getAllManufacturesVehicles(\App\VehicleType::Bus);
     ...
     //////////////////////////////////////////////////////////////////////////////////////
      ...
      $body_type= VehicleBodyType::where('vehicle_type_id', \App\VehicleType::Bus)->get();
      $make=$this->manufacture->getAllManufacturerByType(\App\VehicleType::Bus);
      $city=City::all();
      ...
      $reg_cities=$this->registration_city->getAllRegisterdCityVehicles(\App\VehicleType::Bus);
      $transmissions=$this->transmission->getAllTransmissionVehicles(\App\VehicleType::Bus);
      $engine_types=$this->engine_type->getAllEngineTypeVehicles(\App\VehicleType::Bus);
      $assemblies=$this->assembly->getAllAssemblyVehicles(\App\VehicleType::Bus);
      $colors=$this->color->getAllColorVehicles(\App\VehicleType::Bus);
      $body_types=$this->body_type->getAllBodyTypeVehicles(\App\VehicleType::Bus);
      $models=$this->model->getAllModelVehicles(\App\VehicleType::Bus);
      $city=City::orderby('title','asc')->get();
      $manufacture=$this->manufacture->getAllManufacturerByType(\App\VehicleType::Bus);

//////////////////////////////////////////////////////////////////////
      
      $most_featured_cars= \App\MostFeatureVehicle::where([['vehicle_type', \App\VehicleType::Car],['active',1],['isDeleted',0]])->get();

     $all_busses=$this->vehicle->getAllOthers(\App\VehicleType::Bus);
      $images=array();
      
      
      foreach($all_busses as $a_b)
          {
      ...
      return view('website/Buspages.index',compact('updated_date','most_featured_cars','images','model_year','manufacture','city','checked','new_busses','all_busses','city_vehicles_count','manufacture_busses','reg_cities','transmissions','engine_types','assemblies','colors','body_types','models'));

        
       
    }
    
    public function updateAllBuses(Request $request)
    {
        $filters = [];
        ...
        if($request->has('condition')){
            $filters['condition']=str_replace(',','',$request->condition);
        }
        if($request->has('name'))
        {
        ...
        $all_busses = $this->vehicle->searchOtherByAllFilters($filters,\App\VehicleType::Bus);
   ...
   // dd($cars);
        return view('website/Buspages.update-all-buses',compact('all_busses','images','updated_date'));
     ...
     public function busDetails(Request $request)
    {
     ...
     foreach($data as $a_c)
      ...
      $related_bus=$this->vehicle->getRelatedOtherByModel($request->id,$model_id,\App\VehicleType::Bus);
      

        return view('website/Buspages.detail-bus',compact('data','feature','images','related_bus','updated_date'));

    }
    
    
    public function sellBus()
    {
        ...
        $feature=Feature::where('vehicle_type_id',\App\VehicleType::Bus)->get();
        $reg_city=$this->registration_city->getAllRegCities();
        $make=$this->manufacture->getOtherManufactures(\App\VehicleType::Bus);
        
      return view('website/Buspages.sell-bus',['city'=>$cities,'year'=>$year,'make'=>$make,'engine_type'=>$engine,'transmission'=>$transmission,'assembly'=>$assembly,'feature'=>$feature,'reg_city'=>$reg_city]);
    ...
    public function addBus(Request $request)
    {
        ...
        $ad_id=$this->vehicle->addOther($request,\App\VehicleType::Bus,$code);
    ...
    public function searchBusByAllFilters(Request $request)
   {
